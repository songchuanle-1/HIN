...
#### global
class MultiHeadAttention_TOPK(Module):
    '''
    Multi-head attention layer with Dropout and Layer Normalization.
    '''

    def __init__(self, d_model, d_k, d_v, h, dropout=.1, identity_map_reordering=False, can_be_stateful=False, 
                                        shortcut=True, arr = [1,9,16,25,36,49,64,81],attention_module=None, attention_module_kwargs=None):
        super(MultiHeadAttention_TOPK, self).__init__()
        self.identity_map_reordering = identity_map_reordering
        self.shortcut = shortcut
        if attention_module is not None:
            if attention_module_kwargs is not None:
            ...
            self.attention = ScaledDotProductAttention_TOPK(d_model=d_model, d_k=d_k, d_v=d_v, h=h,arr=arr)
        self.dropout = nn.Dropout(p=dropout)
        self.layer_norm = nn.LayerNorm(d_model)
...
class ScaledDotProductAttention_TOPK(nn.Module):
    '''
    Scaled dot-product attention
    '''

    def __init__(self, d_model, d_k, d_v, h, dropout=.1,arr = [1,9,16,25,36,49,64,81], comment=None):
        '''
        :param d_model: Output dimensionality of the model
        :param d_k: Dimensionality of queries and keys
        :param d_v: Dimensionality of values
        :param h: Number of heads
        '''
        super(ScaledDotProductAttention_TOPK, self).__init__()
        self.fc_q = nn.Linear(d_model, h * d_k)
        self.fc_k = nn.Linear(d_model, h * d_k)
        self.fc_v = nn.Linear(d_model, h * d_v)
        # self.fc_o = nn.Linear(h * d_v, d_model)
        self.fc_o = nn.Linear(d_v, d_model)
        self.fc_alpha = nn.Linear(h * d_k, h * d_k)
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None):
        ...
        ### ###adaptive topk mask
        masks = []
        for i in range(self.h):  ### 每个头不同的尺度
            mask = self.get_scores_mask(att[:, i, :, :], i)
            masks.append(mask)
        attmask = torch.stack(masks, dim=1)
        att = att * attmask
        ###add------------------------
        ...
        ###add------------------------
        dynamic_regions = torch.matmul(att, v)
        out = dynamic_regions.permute(0, 2, 1, 3).contiguous().view(b_s, nq, self.h * self.d_v)  # (b_s, nq, h*d_v)
        dr = self.fc_alpha(out)
        # dr = F.relu(dr)

        ###adaptive group weights
        dr = dr.view(b_s, -1, self.h, self.d_v).transpose(1, 2)
        alpha = torch.sum(dr, dim=(2, 3))
        soft_alpha = torch.softmax(alpha, dim=-1)

        out = dynamic_regions * soft_alpha[:, :, None, None]
        out = torch.sum(out, dim=1)

        out = self.fc_o(out)  # (b_s, nq, d_model)
        ###add------------------------
        return out
    def get_scores_mask(self,score,layer_num):
        # arr = [7,14,20,26,32,38,44,50]
        # arr = [1, 8, 15, 22, 29, 36, 43, 49]
        # arr=[1,1,16,16,36,36,49,49]
        # arr = [1,1,9,9,25,25,49,49]
        
        # 1 
        # arr = [16, 16,16, 16, 16, 16,16, 16]
       #  arr = [1, 1, 4, 4, 9, 9, 16, 25]
       #  arr = [1, 4, 9,16,16,25,36,49]
       #  arr = [1, 9, 16, 16, 25, 25, 36, 49]
       #  arr = [1, 9, 16, 25, 36, 49]
        mask = torch.zeros_like(score)

        vals, indices = torch.topk(score,k=self.arr[layer_num], dim=2)
        mask = mask.scatter_(dim=2, index=indices, value=1)
        return mask

class MultiHeadAttention_TOPK_Concat(Module):
    '''
    Multi-head attention layer with Dropout and Layer Normalization.
    '''
    ...
    def __init__(self, d_model, d_k, d_v, h, dropout=.1, identity_map_reordering=False, can_be_stateful=False, 
                                        shortcut=True,attention_module=None, attention_module_kwargs=None):
        super(MultiHeadAttention_TOPK_Concat, self).__init__()
        self.identity_map_reordering = identity_map_reordering
        self.shortcut = shortcut
        if attention_module is not None:
            if attention_module_kwargs is not None:
            ...
            self.attention = ScaledDotProductAttention_TOPK_Concat(d_model=d_model, d_k=d_k, d_v=d_v, h=h)
        self.dropout = nn.Dropout(p=dropout)
        self.layer_norm = nn.LayerNorm(d_model)
...
class ScaledDotProductAttention_TOPK_Concat(nn.Module):
    '''
    Scaled dot-product attention
    '''
    ...
    def __init__(self, d_model, d_k, d_v, h, dropout=.1, comment=None):
        ...
        :param h: Number of heads
        '''
        super(ScaledDotProductAttention_TOPK_Concat, self).__init__()
        self.fc_q = nn.Linear(d_model, h * d_k)
        self.fc_k = nn.Linear(d_model, h * d_k)
        self.fc_v = nn.Linear(d_model, h * d_v)
        self.fc_g = nn.Linear(d_model, h * d_v)
        # self.fc_o = nn.Linear(h * d_v, d_model)
        self.fc_o = nn.Linear(h * d_v, d_model)
        self.fc_alpha = nn.Linear(h * d_k, h * d_k)
        self.dropout = nn.Dropout(dropout)
        ...
        self.d_v = d_v
        self.h = h
        # self.threshold_scale = nn.Parameter(torch.ones(h,dtype=torch.float32)*0.5, requires_grad=True)
        # self.threshold_scale = nn.Parameter(torch.randn(h,dtype=torch.float32), requires_grad=True)
        # self.threshold_scale = nn.Parameter(torch.randn(1,h), requires_grad=True)
    ...
    def forward(self, queries, keys, values, attention_mask=None, attention_weights=None):
        ...
        k = self.fc_k(keys).view(b_s, nk, self.h, self.d_k).permute(0, 2, 3, 1)  # (b_s, h, d_k, nk)
        v = self.fc_v(values).view(b_s, nk, self.h, self.d_v).permute(0, 2, 1, 3)  # (b_s, h, nk, d_v)
        g = self.fc_g(values).view(b_s, nk, self.h, self.d_v).permute(0, 2, 1, 3)  # (b_s, h, nk, d_v)
        att = torch.matmul(q, k) / np.sqrt(self.d_k)  # (b_s, h, nq, nk)
        ...
        attmask = self.get_scores_mask(att,g)
        att = att * attmask
        ###add------------------------
        ...
        return out
    def get_scores_mask(self, scores, g):
        # 动态计算 top-k
        b_s, h, nq, nk = scores.size()
        g1 =self.l1(g).squeeze(-1).unsqueeze(2)
        g1=g1.repeat(1,1,nq,1)
        # print(g1.size())
        # print(scores.size())
        # print(g1[0,0])
        # pp
        ...
        for i in range(h):
            head_scores = scores[:, i, :, :]  ##[bs,nq1,nq2]
            thre = g1[:, i, :, :]
            # print(head_scores.size())
            # pp
            # mean = head_scores.mean(dim=-1, keepdim=True)
            # std = head_scores.std(dim=-1, keepdim=True)
            # print()
            # Calculate threshold dynamically
            # threshold = mean - self.threshold_scale[i] * std
            # print(mean)
            # print(std)
            # print(torch.sigmoid(threshold))
            # Normalize scores using softmax
            normalized_scores = F.softmax(head_scores, dim=-1)

            # Create mask based on the normalized scores and threshold
            # mask = (normalized_scores >= torch.sigmoid(threshold)).float()
            mask = (normalized_scores >=thre).float()
            masks[:, i, :, :] = mask
        return masks
