...
# from common.data.dataset_online import COCODataset
import data.evaluation as evaluation
# import evaluation_caption as evaluation
...
def evaluate_metrics_test(model, dataloader, text_field, epoch, device = 'cuda',spice=False, args=None):
    import itertools
    model.eval()
    gen = {}
    gts = {}
    ...
    gts = evaluation.PTBTokenizer.tokenize(gts)
    gen = evaluation.PTBTokenizer.tokenize(gen)
    scores, _ = evaluation.compute_scores(gts, gen, spice)
    return scores




def evaluate_metrics_test_base_and_paper(model, dataloader, text_field, epoch, device = 'cuda',spice=False, args=None):
    import itertools
    model.eval()
    gen = {}
    gts = {}
    ...
    images_ids =[]
    with tqdm(desc='Epoch %d - evaluation' % epoch, unit='it', total=len(dataloader)) as pbar:
    # with tqdm(desc='Epoch %d - evaluation' % epoch, unit='it', total=2) as pbar:

        for it, (images,json_info) in enumerate(iter(dataloader)):
        
            caps_gt =[]
            for a in range(len(json_info)):
                json_info_list = json_info[a]
                one_img_gt = []
                for b in range(len(json_info_list)):
                    json_dict = json_info_list[b]
                    one_img_gt.append(json_dict['caption'])
                caps_gt.append(one_img_gt)
            ...
            for i, (gts_i, gen_i) in enumerate(zip(caps_gt, caps_gen)):
                ...
                json_info_list = json_info[i]
                images_id = json_info_list[0]['image_id']
                images_path = json_info_list[0]['image_path']
                images_ids.append(images_id)
                gt_jsons[images_id]={}
                gt_jsons[images_id]['gt'] = gts_i
                gt_jsons[images_id]['image_path'] = images_path

                gen_jsons[images_id]={}
                gen_jsons[images_id]['predict'] = gen_i.strip()
                gen_jsons[images_id]['image_path'] = images_path

                gt_gen_jsons[images_id]={}
                gt_gen_jsons[images_id]['gt'] = gts_i
                gt_gen_jsons[images_id]['predict'] = gen_i.strip()
                gt_gen_jsons[images_id]['image_path'] = images_path
    ...
    gts = evaluation.PTBTokenizer.tokenize(gts)
    gen = evaluation.PTBTokenizer.tokenize(gen)
    scores, scores_every = evaluation.compute_scores(gts, gen)
    Ciders=scores_every['CIDEr']
    print(scores)


    for p in range(len(images_ids)):
        gen_jsons[images_ids[p]]['Cider'] = Ciders[p]      
        gt_gen_jsons[images_ids[p]]['Cider'] = Ciders[p]    
    


    return Ciders,gen_jsons,gt_jsons,gt_gen_jsons

def evaluate_metrics_test_online(model, dataloader, text_field, epoch, device = 'cuda',spice=False, args=None):
    import itertools
    model.eval()
    result = []
    with tqdm(desc='Epoch %d - evaluation' % epoch, unit='it', total=len(dataloader)) as pbar:
        for it, (images, image_ids) in enumerate(iter(dataloader)):
            ...
            for i, (image_id, gen_i) in enumerate(zip(image_ids, caps_gen)):
                gen_i = ' '.join([k for k, g in itertools.groupby(gen_i)])
                # print("image_id:{}".format(image_id[0]), "caption:{}".format(gen_i.strip()))
                result.append({"image_id": int(image_id[0].item()), "caption": gen_i.strip()})
