...
from data.evaluation import PTBTokenizer, Cider
# from evaluation_caption import PTBTokenizer, Cider
...
def train(args, model, datasets, image_field, text_field, optim=None, scheduler=None, 
    ...
    cider_cache_path = args.cider_cache_path
    if use_rl:
    ...
    #     return args.xe_base_lr* (decay_rate ** (s / decay_steps))
    def lambda_lr_rl(s):
        refine_epoch = 8
        if s <= refine_epoch:
            lr = args.rl_base_lr
    ...
    last_saved = os.path.join( output, '%s_last.pth' % args.exp_name)
    best_saved = os.path.join(output, '%s_best.pth' % args.exp_name)
    best_test_saved= os.path.join(output, '%s_best_test.pth' % args.exp_name)
    
    if args.resume_last or args.resume_best:
        if use_rl:
            last_saved = os.path.join(output, '%s_rl_last.pth' % args.exp_name)
            best_saved = os.path.join(output, '%s_rl_best.pth' % args.exp_name)
            best_test_saved= os.path.join(output, '%s_rl_best_test.pth' % args.exp_name)
        if args.resume_last:
            fname = last_saved
        else:
            fname = best_saved
        ...
        data = torch.load(best_test_saved, map_location=device)
        model.load_state_dict(data['state_dict'], strict=False)
        best_cider = data['best_cider']
        start_epoch = 0
        patience = 0
        ...
        best_test_saved= os.path.join('coco/checkpoints', output, '%s_rl_best_test.pth' % args.exp_name)
    ...
    for e in range(start_epoch, start_epoch + 100):
        ...
        if best_test:
            copyfile(last_saved, best_test_saved)
        if exit_train:
            print('{}:best_test_cider:{}'.format(args.exp_name,best_test_cider))
            writer.close()
            break