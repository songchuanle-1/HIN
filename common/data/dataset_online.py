...
class DictionaryDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        #     images_id.append(item['image_id'])
        # return self.key_dataset[i], captions, images_id
...
class OnlineTestDataset(Dataset):
    def __init__(self,ann_path, fields):
        ...
        self.info_dict_cap= ann_path
        self.examples = self.get_samples()
        super(OnlineTestDataset, self).__init__(self.examples, fields)
    ...
    def get_samples(self):
        ...
        coco_dataset = pyCOCO(self.info_dict_cap).dataset
        ids = range(len(coco_dataset['images']))
        # bp = len(ids)

        for index in range(len(ids)):

            ann_id = ids[index]
            ann = coco_dataset['images'][ann_id]
            # caption = ann['caption']
            img_id = ann['id']
            # image_path = os.path.join(img_root, ann['file_name'])
            image_path =  ann['file_name']
            orig_size = (ann['width'], ann['height'])

            image_des = {'image_id': img_id, 'image_path': image_path, 'split': 'test', 'orig_size': orig_size}
            # image_des = {'image_id': img_id, 'image_path': image_path, 'split': 'val', 'orig_size': orig_size}
            # text = {'image_id': img_id, 'ann_id': ann_id, 'caption': caption}
            example = Example.fromdict({'image': image_des, 'text':img_id})
    ...
    def get_samples(cls, roots, ids_dataset=None):
        ...
        for split in ['train', 'val', 'test']:
            ...
            # for index in range(100):
                ...
                example = Example.fromdict({'image': image_des, 'text': caption,'id':img_id})
