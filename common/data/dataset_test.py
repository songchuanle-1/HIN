    ...
    def collate_fn(self):
        def collate(batch):
            ...
            for field, data in zip(self.fields.values(), batch):
                ...
                # from  common.data.field import OtherField
                # from  common.data.field import DualImageField
                # # print(1,isinstance(field,RawField))
                # # print(2,isinstance(field,DualImageField))
                # if isinstance(field,DualImageField):
                # # print(data)
                #     tensor = field.process(data)
                #     # print(tensor)
                #     tensors.append(tensor)
                # elif isinstance(field,OtherField):
                #     # print(data)
                #     tensor = field.process(data)
                #     # print(tensor)
                #     for i in range(len(tensor)):
                #         tensors.append(tensor[i])
                    # tensors.append(data['image_id'])
                    # tensors.append(data['caption'])
                # print(tensors)
...
class DictionaryDataset(Dataset):
    ...
    def __getitem__(self, i):
        ...
        # return self.key_dataset[i], captions,imageids
...
class COCODataset(PairedDataset):
    def __init__(self, image_field, text_field, img_root, ann_root, id_root=None, use_restval=True,
        ...
        roots['val'] = {
            ...
            # 'cap': '/speedup_data/slurm_data/songcl_data_place/DFT/outputs/captions_val2014.json'
        }
        roots['test'] = {
            'img': os.path.join(img_root, 'val2014'),
            'cap': os.path.join(ann_root, 'captions_val2014.json')
            # 'img': os.path.join(img_root, 'test2014'),
            # 'cap': '/speedup_data/slurm_data/songcl_data_place/DFT/annotations/image_info_test2014.json'
        }
        roots['trainrestval'] = {
    ...
    def get_samples(cls, roots, ids_dataset=None):
        ...
        for split in ['train', 'val', 'test']:
            ...
            # for index in range(100):
                ...
                text = {'image_id': img_id, 'ann_id': ann_id, 'caption': caption,'image_path': image_path, 'split': split, 'orig_size': orig_size}
                # print(text)
                example = Example.fromdict({'image': image_des, 'text': text})
                # example = Example.fromdict({'image': image_des, 'text': caption})
